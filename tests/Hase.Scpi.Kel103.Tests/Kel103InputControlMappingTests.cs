...
{
    [Theory]
    [InlineData(0, "Input.Activate", true, ":INPut ON")]
    [InlineData(1, "Input.Deactivate", false, ":INPut OFF")]
    ...
    [Fact]
    public void All_UsesUniquePathsAndStatesAndIsReadOnly()
    {
        Assert.Equal(
        ...
        Assert.Equal(
            Kel103InputControlMapping.All.Count,
            Kel103InputControlMapping.All.Select(mapping => mapping.InputEnabled).Distinct().Count());
