...
public static class Kel103ControlledSetpointDefinition
{
    public static DescriptorReference Reference { get; } =
        new(Kel103IdentityDefinition.Reference.Id, version: 4);
    ...
    private static EndpointDescriptorDefinition Create()
    {
        ...
        CommandDescriptor ModeCommand(string path, string name) => new(
            ...
            new InstrumentKind("ElectronicLoad"))
        {
            ...
            Interface = new InstrumentInterface(
                ...
                commands:
                [
                    ModeCommand("Mode.SelectConstantCurrent", "Select constant-current mode"),
                    ModeCommand("Mode.SelectConstantVoltage", "Select constant-voltage mode"),
                    ModeCommand("Mode.SelectConstantResistance", "Select constant-resistance mode"),
                    ModeCommand("Mode.SelectConstantPower", "Select constant-power mode"),
                    ModeCommand("Mode.SelectShortCircuit", "Select short-circuit mode")
                ])
        };
        ...
        return new EndpointDescriptorDefinition(
            new EndpointMetadata
            {
                ...
                Description = "Controlled KEL-103 setpoint and mode-selection endpoint definition."
            },
            [instrument]);
    }
}
