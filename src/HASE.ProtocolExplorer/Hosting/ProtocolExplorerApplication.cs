...
/// <summary>
/// Runs the protocol explorer over the generic scenarios this repository
/// ships, plus any an entry point composes into it.
/// </summary>
/// <remarks>
/// The explorer knows no instrument. A composition root that ships
/// instrument characterization supplies those scenarios and their usage
/// lines here, exactly as the Runtime Host composes endpoint providers.
/// </remarks>
public static class ProtocolExplorerApplication
{
    public static int Run(
        string[] args,
        IEnumerable<IScenario>? additionalScenarios = null,
        IReadOnlyList<string>? additionalUsageLines = null)
    {
        WriteHeader();
        ...
        if (args.Length == 0)
        {
            WriteHelp(additionalUsageLines);
            return 1;
        }
    ...
    private static void WriteHelp(
        IReadOnlyList<string>? additionalUsageLines)
    {
        Console.WriteLine(
            "Usage:");
        Console.WriteLine();
        ...
        Console.WriteLine(
            "  c026 esp32 <host>");
        Console.WriteLine(
            "  c026 arduino [baud rate] [verification timeout seconds]");
        Console.WriteLine(
            "  c027 esp32 <host>");
        Console.WriteLine(
        ...
        foreach (string line in additionalUsageLines ?? [])
        {
