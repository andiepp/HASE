...
public sealed partial class PythonObservationAuthorizer
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (PathComparer.Equals(policy, rollback) || !File.Exists(policy)
            ...
            || !Directory.Exists(parent)) throw Failure("authorization-target-invalid");
        ...
        try
        {
            ...
            if (HashFile(policy) != request.ExpectedAuthorizationPolicySha256)
                throw Failure("authorization-input-revision-mismatch");
            File.Replace(stage, policy, rollback, false); published = true;
        ...
        catch (PythonObservationAuthorizationException)
        {
            if (published) Rollback(policy, rollback,
                request.ExpectedAuthorizationPolicySha256); throw;
        }
    ...
    private static byte[] Candidate(byte[] original)
    {
        try
        {
                ...
                new JsonWriterOptions { Indented = true }))
            {
                ...
                writer.WriteStartObject(); writer.WriteString("principalId", Principal);
                writer.WriteString("permission", "observation.subscribe");
                writer.WriteEndObject(); writer.WriteEndArray(); writer.WriteEndObject();
            }
            byte[] result = stream.ToArray();
            _ = RuntimeHostAuthorizationPolicyFile.Load(result); return result;
        }
