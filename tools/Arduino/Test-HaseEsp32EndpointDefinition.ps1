...
$expectedCommit =
    "e7f4e32eb504e35f0e3e44464f6ea9a1d46ed46c"
...
$expectedChangedPaths = @(
    "libraries/HaseEsp32Endpoint/src/HaseApplicationCallbacks.h",
    "libraries/HaseEsp32Endpoint/src/HaseEndpointDefinition.h",
    "libraries/HaseEsp32Endpoint/src/HaseEndpointDefinitionValidator.h",
    "libraries/HaseEsp32Endpoint/src/HaseEsp32Endpoint.h",
    "tests/Arduino/HaseEndpointDefinitionValidation/HaseEndpointDefinitionValidation.ino",
    "tools/Arduino/Test-HaseEsp32EndpointCompilation.ps1",
    "tools/Arduino/Test-HaseEsp32EndpointDefinition.ps1"
) | Sort-Object
...
{
    throw "The 54C1 evidence directory already exists."
}
...
{
    throw "Repository HEAD is not the approved 54B1 baseline."
}

if ($origin.Count -ne 1 -or $origin[0].Trim() -cne $expectedCommit)
{
    throw "origin/main is not the approved 54B1 baseline."
}
...
{
    throw "54C1 files must not be staged during focused validation."
}

$repositoryFixtureRoot =
    Join-Path `
        $RepositoryRoot `
        "tests\Arduino\HaseEndpointDefinitionValidation"

$repositoryFixturePath =
    Join-Path `
        $repositoryFixtureRoot `
        "HaseEndpointDefinitionValidation.ino"

if (-not (Test-Path -LiteralPath $repositoryFixturePath -PathType Leaf))
{
    throw "The endpoint-definition validation fixture is missing."
}
...
$stagedFixtureRoot =
    Join-Path `
        $EvidenceRoot `
        "Sketch\HaseEndpointDefinitionValidation"
...
if ($stagedFixtureFiles.Count -ne 1 `
    -or $stagedFixtureFiles[0].Name -cne
        "HaseEndpointDefinitionValidation.ino")
{
    throw "The staged endpoint-definition fixture is invalid."
}
...
Write-Host "Starting endpoint-definition compilation 1 of 2..."
$compileOneResult =
    Invoke-CliCapture `
...
{
    throw "The first endpoint-definition compilation failed."
}
...
Write-Host "Starting endpoint-definition compilation 2 of 2..."
$compileTwoResult =
    Invoke-CliCapture `
...
{
    throw "The second endpoint-definition compilation failed."
}
...
{
    throw "The focused endpoint-definition compilation produced warnings."
}
...
Write-Host ""
Write-Host "ADR-0054 Increment 54C1 focused definition validation"
Write-Host ""
...
Write-Host "Compile-time fixtures       :" "valid and rejected"
