...
try{
 ...
 if(Test-Path -LiteralPath $rollback){throw"rollback"};if(-not(Test-Path -LiteralPath (Split-Path -Parent $rollback) -PathType Container)){throw"rollback"}
 ...
 $utf8=[Text.UTF8Encoding]::new($false);[IO.File]::WriteAllText($rollback,($e|ConvertTo-Json -Depth 4),$utf8)
 ...
 $candidate=($doc|ConvertTo-Json -Depth 8);$stage=$profile+".stage-"+[Guid]::NewGuid().ToString("N");[IO.File]::WriteAllText($stage,$candidate,$utf8);[IO.File]::Replace($stage,$profile,$null);$published=$true
