[CmdletBinding()]
param([Parameter(Mandatory=$true)][string]$CleanupDirectory,[Parameter(Mandatory=$true)][string]$ExpectedComputer)
$ErrorActionPreference="Stop";Set-StrictMode -Version Latest
try{
 if($env:COMPUTERNAME-cne$ExpectedComputer-or@(Get-Process Hase.DesktopHost.App,Hase.Client.Wpf.App -ErrorAction SilentlyContinue).Count-ne0){throw"preflight"}
