...
$policyReady = Test-Path -LiteralPath $AuthorizationPolicyPath -PathType Leaf
$rollbackAvailable = -not (Test-Path -LiteralPath $RollbackPath)
if (-not $policyReady) { Write-Error "Python observation authorization failed: inputs-invalid."; exit 1 }
if (-not $rollbackAvailable) { Write-Error "Python observation authorization failed: inputs-invalid."; exit 1 }
...
& dotnet run --project (Join-Path $root "src\Hase.Python.CredentialProvisioning.Operator") `
    -c Release --no-build -- authorize-observation `
