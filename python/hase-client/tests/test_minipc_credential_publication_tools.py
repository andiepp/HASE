...
def test_application_profile_is_committed_last():
 s=src("Publish-HaseMiniPcPythonCredential.ps1");assert s.index('status="credential-published"') < s.index("[IO.File]::Replace") < s.index('status="committed"')
...
def test_recovery_restores_sources_and_removes_only_new_targets():
 s=src("Recover-HaseMiniPcPythonCredentialPublication.ps1");assert "enrollment.original" in s;assert "application-profile.original" in s;assert 'foreach($f in @($cert,$key,$profile,$policy))' in s;assert "Remove-HaseMiniPcPythonClientAuthority" not in s
